...
SParams = Tuple[int, int]						# Parametros de S: (c: int, n: int)
...
def knapsack_memo_path(w: List[int], v: List[int], C: int) -> Solution: 		# VERSION EXAMEN
	def S(c: int, n: int) -> Score:			# COSTE ESPACIAL: O(C x N)
		if n == 0: return 0					# COSTE TEMPORAL = rellenar una celda es O(1) porque no hay bucle for, entonces
		if (c, n) not in mem:				# coste temp es O(C x N) = coste espacial
			if w[n-1] <= c:
